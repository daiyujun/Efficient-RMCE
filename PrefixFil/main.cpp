...
int zese=1;
int lgrp;
void cliSearch(VI &C, VI &cand, VI &prev, double pprod,int dh,int mindh)
{
    if(dh>=mindh) return ;
    //if(!zese&&dh>=lgrp) return ;
    ...
    for (int i=0; i<doing.size(); ++i)
    {
      ...
      // cout<<lese<<endl;
        ...
        cliSearch(C, candt, prevt, pprod*pr,dh+1,min(mindh,lese));
        if(zesee) zese=1;
        unit[0]=v;
        C= C-unit;
        prev= prev+unit;
...
int main(int argc, char **argv)
{
    ...
    if (argc<6) {
        ...
        //inputss.txt  0.5  D  G  outasdasdput5.txt
        exit(1);
    }
    char ses[50]="Ainput3.txt";
    list2adj(argv[1],ses);
    cout<<"input yes"<<endl;
    g.build(ses);
    tau = atof(argv[2]);
    ...
    for (int ci=0; ci<odoing.size(); ++ci) {
        ...
        oc.clear();
        oc.push_back(i);
        cliSearch(oc, ocand, oprev, 1.0,1,1e5);
    }
   ...
   // holder.topk(10);
    ...
    cout<<"the time is: "<<(double)(end-star)/CLOCKS_PER_SEC<<" s "<<endl;
    fclose(fcliques);
    free(lookup);
    return 0;
}
