...
int setin(int first1,int last1,int first2, int last2,int &v,VI &a,VI &b)
{
    int result=0;
    int z=1;
...
bool CliqueHolder::passFilter(VI &C) {
    ...
    for (it=itup; it!= lv.end(); ++it) {
        ...
        VI P=cli;
        sort(P.begin(), P.end());
        des=setin(upovl,las,upovl,P.size(),lav,C,P);//后缀取交集，lav是第一个位置
        if(des)
        {
            f1=P.front();e1=P.back();
            if (min(e1,e2)-max(f1,f2)<=upovl &&setin(0,upovl,0,lav,lav,C,P)>=upovl-des)//前缀取交集
            {
                pass=false;
                break;
            }
